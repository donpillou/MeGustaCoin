...
public:
  ...
  public:
    void delegateEntity(Entity& entity)
    {
      ...
      QHash<quint64, Entity*>::iterator it = table.entities.find(id);
    ...
    {
      quint64 replacedId = replacedEntity.getId();
      EntityTable& replacedTable = tables[replacedEntity.getType()];
      QHash<quint64, Entity*>::iterator itReplaced = replacedTable.entities.find(replacedId);
      ...
      quint64 newId = entity.getId();
      EntityTable& newTable = tables[entity.getType()];
      QHash<quint64, Entity*>::iterator it = newTable.entities.find(newId);
    ...
    void removeEntity(quint32 type, quint64 id)
    {
    ...
    template <typename E> void removeEntity(E eType, quint64 id)
    {
      removeEntity((quint32)eType, id);
    }

    template <typename C> void removeEntity(quint64 id)
    {
    ...
    Entity* getEntity(quint32 type, quint64 id) const
    {
      ...
      QHash<quint64, Entity*>::ConstIterator it = table.entities.find(id);
      if(it == table.entities.end())
        return 0;
      return *it;
    }

    template<class C> C* getEntity(quint64 id) const
    {
    ...
    template<class C> void getAllEntities(QList<C*>& entities) const
    {
      ...
      if(itTable != tables.end())
      {
        const QHash<quint64, Entity*>& hashMap = itTable->entities;
        for (QHash<quint64, Entity*>::ConstIterator i = hashMap.begin(), end = hashMap.end(); i != end; ++i)
          entities.append(dynamic_cast<C*>(*i));
      }
    }

    template<class C> quint64 getNewEntityId()
    {
      EntityTable& table = tables[(quint32)C::eType];
      if(table.entities.isEmpty())
        return 1;
      quint64 id = (--table.entities.end()).value()->getId();
  ...
  private:
    struct EntityTable
    {
      ...
      {
        //qDeleteAll(entities);
        for(QHash<quint64, Entity*>::Iterator i = entities.begin(), end = entities.end(); i != end; ++i)
        {
          Entity* e = *i;
          delete e;
        }
...
public:
  template <typename E> Entity(E type, quint64 id) : type((quint32)type), id(id) {}
  virtual ~Entity() {}
